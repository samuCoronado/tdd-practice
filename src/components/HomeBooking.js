...
const HomeBooking = (props) => {
    ...
    useEffect(() => {
      ...
      if(Number.isInteger(total)) setTotalPriceState(total);
      else setTotalPriceState('--');

    }, [checkInState, checkOutState, props]);

    const handleBooking = () => {
      apiClient.bookHome(props.home, checkInState, checkOutState).then((response) => console.log(response))
    }

    if(props.home === null) return <div data-testid="empty"></div>
    ...
    return (
        <div>
           ...
           <button data-testid="book-btn" onClick={handleBooking}>Book</button>
        </div>
    )
}
